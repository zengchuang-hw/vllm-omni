...
"""Run a trained MammothModa2.5 checkpoint using its Hunyuan tokenizer."""
...
def main():
    ...
    parser.add_argument("--output", default="mammoth_moda25.png")
    parser.add_argument("--deploy-config", default="vllm_omni/deploy/mammoth_moda25.yaml")
    args = parser.parse_args()
    config = AutoConfig.from_pretrained(args.model)
    if not isinstance(config, MammothModa25Config):
        raise ValueError("--model must be a trained MammothModa2.5 checkpoint")
    tokenizer = AutoTokenizer.from_pretrained(args.model)
    prompt = build_text_to_image_prompt(tokenizer, config, args.prompt, args.height, args.width)
